...
if (!window.isolatedContentListenerAdded) {
  ...
  window.addEventListener("message", (event) => {
    if (event.source === window && event.data.type === "FROM_GLOBAL") {
      eventmethod = event.data.method; //Save event method
      response = event.data.data; //Save event data
...
function sendResponseToExtension(msg,sender,sendResponse){
    ...
    if(msg == 'REQUEST_DATA'){
      console.log("DATA to be sent to extension: ",{calldata:response,chaindata,method:eventmethod});
